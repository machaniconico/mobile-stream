...
- Still-image VTuber face tracking with native-camera input on mobile, simulated input on web, calibration, dead-zone filtering, jump limiting, lost-face return tuning, image-aspect-aware geometry rig inference, browser alpha-mask foreground auto-rig analysis for transparent/padded PNGs, production rig-line diagnostics, rig quality score/grade gating, native 2.5D motion scaling, tunable illustration rig regions, lightweight pseudo mesh deformation, hair/body follow-through, and eye/mouth deformation in the iOS/Android compositor.
- IRIAM-style single-illustration animation is now covered by the still-image 2.5D tracker foundation plus image-aspect-aware geometry auto rigging, alpha-mask foreground analysis for padded still images, and tunable lightweight pseudo deformation; true semantic image-segmentation-based mesh/depth rigging, explicit eye/mouth part extraction, and physical-device tuning against native-camera tracking remain commercial-grade continuous improvements.
