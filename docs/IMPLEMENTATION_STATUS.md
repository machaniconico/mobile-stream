...
- Release evidence package tooling that copies a passed RC report, support bundle, generated artifacts, distribution manifests, platform dashboard evidence, and store-submission evidence into a standalone hash-verified archive directory for commercial handoff and store-submission audit, fails closed when commercial distribution/dashboard/store-submission artifact groups or manifest-referenced payloads are missing, and scans package-level text evidence for unredacted OAuth tokens, stream keys, client secrets, device codes, and Authorization headers.
