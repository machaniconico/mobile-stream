...
- Saved release-candidate report audit that revalidates archived RC reports against the current commit, clean git state, support-bundle SHA-256 plus the support bundle's commercial release gate at the report timestamp, loopback browser UI evidence, generated artifact hashes, release-configuration input hashes, and required gate outcomes before commercial approval.
