...
`npm run verify:repo-automation` audits GitHub Actions release gates and checks that CI keeps restricted token permissions, Java/Android SDK provisioning, release automation script verification, native release checks, tests, build, bundle-size, RN bundle coverage, Android native build coverage, and PR auto-merge remains explicit opt-in through the `automerge` label.
