...
- Completed stream session summaries retain safe native runtime evidence, including publisher state, compositor status, still-image asset loaded/missing counts, VRM pose payload active/missing counts, congestion, queue size, sent/dropped frame counts, and bytes, without storing raw native error messages.
...
- VRM/VRoid runtime pose mapping converts shared avatar motion into native-consumable humanoid bone rotations, expression/blendshape weights, look-at values, root offsets, render-graph payloads, native runtime pose-delivery telemetry, validation evidence, and support-bundle pose summaries.
