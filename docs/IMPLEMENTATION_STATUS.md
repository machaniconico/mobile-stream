...
- Face-tracking production diagnostics for prepared PNGTuber assets, simulated/native-camera input risk, tracking runtime state, runtime frame freshness, native face-landmark confidence/readiness, active motion count, still-image rig quality score/grade, current-runtime Go Live preflight warnings, public launch start-lock evidence, support reports, and commercial validation checklist evidence.
...
- Physical validation evidence now records explicit physical-device identity proof, rejects Simulator/Emulator/browser/generic test-device labels, surfaces physical iOS/Android coverage in Web/React Native diagnostics, and requires support bundle v41 physical-device plus current destination/protocol scope, passing private launch rehearsal, stable monitor-hold sample/duration/drop/reconnect, controlled weak-network quality automation, native frame/byte/frame-interval/compositor applied/skipped overlay proof plus loaded/decoded/composited still-image asset pixel proof, VRM renderer/model/pose proof when VRM sources are present, mic/headphone native monitor write/drop/latency plus Bluetooth review proof when applicable, fresh avatar-tracking runtime, ready native face-landmark confidence, retained manifest PNGTuber prepared-count/rig-quality or VRM visible-count/native-renderer proof, spoken chat-readout, YouTube/Twitch dashboard identity/state manifest proof including Twitch title/category/language metadata, and same-run platform ingest proof before commercial release approval.
...
- Redacted support bundle export combining preflight, diagnostics, face-tracking runtime freshness, scene composition, native still-image asset load/decode pixel evidence, stream target, and safe profile summaries for support triage.
...
- Web and React Native validation evidence panels show latest-run monitor-hold and native runtime evidence, including native overlay applied/skipped counts, so release-candidate records can prove the stable private stream and native publisher/compositor path.
...
- iOS ReplayKit Broadcast Upload Extension compositor for PNGTuber still-image/fallback, text, chat, image, and solid overlays on encoded frames, with App Group runtime composition status and still-image asset load/miss/decode/composited pixel evidence, including separate App Group-copied still-image loaded/decoded/composited proof required by v41 release evidence.
...
- iOS physical-device execution of the v41 App Group still-image render-proof run against real devices and real ingest endpoints.
