...
const sensitiveStructuredHeaderPattern =
  /((?:\b|["'])(?:Authorization|x-api-key|api-key|client-secret|stream-key|oauth-token|auth-token|bearer-token|access-token|refresh-token|id-token|device-code|user-code)(?:\b|["'])\s*\]?\s*[:=]\s*["'])([^"'\n]+)(["'])/gi;
...
export const redactSensitiveText = (value: string): string => {
  ...
  return value
    ...
    .replace(sensitiveStructuredHeaderPattern, `$1${redacted}$3`)
