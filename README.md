...
- Public YouTube and Twitch start locks block checklist warnings as well as failures, including stale dashboard evidence, disabled chat readout, unsafe/muted mic monitoring, incomplete commercial evidence, and mock-engine fallback, so a release build cannot silently treat an unverified setup as a real platform-visible stream.
