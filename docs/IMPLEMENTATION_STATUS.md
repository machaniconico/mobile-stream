...
- Store submission draft/checklist tooling for App Store / Play Console metadata JSON, human review Markdown, and iOS/Android store screenshots, including metadata/review generation from real-device screenshots or canonical workspace UI evidence, screenshot source tracking, final-submission real-device screenshot gating, required listing/privacy/data-safety checks, byte/SHA-256 recording, git commit/dirty-state provenance, secret-like text detection, verification, and optional release-candidate/report artifact capture when the checklist exists.
