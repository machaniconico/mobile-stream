...
- Physical validation native-runtime proof must match the current scene's native composition requirements, so a screen-only runtime snapshot cannot approve a PNGTuber/image/text/chat overlay scene, native overlay totals without the expected text/chat applied-kind proof cannot approve subtitle/comment output, and incomplete or non-production VRM renderer backend/model/pose telemetry cannot approve a VRM/VRoid scene for release evidence.
