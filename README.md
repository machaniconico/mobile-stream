...
- Release-candidate verification command that chains source gates, browser UI verification, React Native bundle generation, and the commercial support-bundle gate into one local approval step.
...
`npm run verify:release-candidate -- /path/to/support-bundle.json` is the local commercial release-candidate approval command. It runs native release-config checks, unit tests, web/RN typechecks, the web build, iOS/Android Metro bundles, browser UI verification, and the commercial support-bundle gate. Use `--ui-url=http://127.0.0.1:5173/` when a preview server is already running. Use `--skip-ui` only when Chrome is unavailable and `npm run verify:ui` has already been run separately.
