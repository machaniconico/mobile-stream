...
- Store submission approval gate that verifies a saved release-candidate report, Android `.aab` / iOS `.ipa` distribution evidence, fresh YouTube/Twitch dashboard screenshot plus status JSON evidence, fresh store-release orchestration report evidence, final store-submission checklist, real-device screenshot source labels, and complete capture of metadata/review/screenshot artifacts in the RC report before App Store / Play Console approval.
