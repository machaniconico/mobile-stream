...
- Physical validation evidence now retains stable monitor-hold bitrate/FPS history, mic FX/headphone monitor snapshots, native self-monitor write/drop proof, device output-route/headphone safety, lip-sync/audio-meter sample summaries, YouTube/Twitch chat readout snapshots, spoken-chat success/failure counts, native face-landmark confidence/readiness, native publisher/compositor telemetry, VRM renderer/model/pose proof when VRM sources are present, fresh YouTube/Twitch dashboard checked-at proof, dashboard broadcast/stream/channel identity and YouTube privacy proof, same-run platform ingest proof that combines native sent video/audio frames with destination receiving state plus checked-at/run-time consistency, stable run/evidence fingerprints, and a retained-run manifest for support-bundle audit correlation, and downgrades retained pass attempts when stable hold, native face-landmark proof, native publisher/compositor proof, VRM renderer proof, native self-monitor proof, audio monitoring, chat readout validation, destination dashboard validation, or same-run ingest handoff validation is incomplete.
...
- Physical validation native-runtime proof must match the current scene's native composition requirements, so a screen-only runtime snapshot cannot approve a PNGTuber/image overlay scene and incomplete VRM renderer/model/pose telemetry cannot approve a VRM/VRoid scene for release evidence.
