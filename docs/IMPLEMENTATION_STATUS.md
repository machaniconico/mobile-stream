...
- VRM/VRoid source records now persist VRM/GLB model URIs, expose them in Web/React Native inspectors, can pick/prepare local `.vrm`/`.glb` files into native-readable storage, warn on missing/remote/relative/unsupported or non-model URIs, and include shared plus native binary GLB validators that detect VRM 1.0/0.x extensions, humanoid bone mappings, expression/blendshape metadata, native runtime load evidence, and pose-to-model compatibility counts.
- VRM/VRoid runtime pose mapping converts shared avatar motion into native-consumable humanoid bone rotations, expression/blendshape weights, look-at values, root offsets, render-graph payloads, native runtime pose-delivery telemetry, pose-to-model compatibility telemetry, renderer readiness evidence, validation evidence, and support-bundle pose summaries.
