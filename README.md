...
- Release evidence package creation that copies a passed clean RC report, support bundle, UI/RN/web artifacts, distribution manifests, dashboard evidence, and store-submission evidence into a standalone hash-verified archive directory for handoff or store-submission audit, rejects development-only dirty-worktree or commit-mismatch RC reports, re-runs the archived support bundle's commercial release gate at the RC report timestamp with the report's warning-approval setting, fails closed when commercial distribution/dashboard/store-submission artifact groups or manifest-referenced payloads are missing, and runs a package-level scan for unredacted OAuth tokens, stream keys, client secrets, device codes, and Authorization headers in text evidence.
