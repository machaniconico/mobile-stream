...
- Distribution artifact manifest tooling for generated Android `.aab` and iOS `.ipa` files, including byte/SHA-256 recording, minimum-size and ZIP-container validation, verification, and optional release-candidate/report artifact capture when the manifest exists.
