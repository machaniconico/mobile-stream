...
- Face-tracking production diagnostics for prepared PNGTuber assets, simulated/native-camera input risk, tracking runtime state, active motion count, current-runtime Go Live preflight warnings, public launch start-lock evidence, support reports, and commercial validation checklist evidence.
...
- Physical validation evidence now records explicit physical-device identity proof, rejects Simulator/Emulator/browser/generic test-device labels, surfaces physical iOS/Android coverage in Web/React Native diagnostics, and requires support bundle v15 physical-device manifest proof before commercial release approval.
...
- IRIAM-style high-fidelity single-illustration auto-rigging beyond the current tunable lightweight still-image 2.5D deformation, including automatic face/body part inference, true mesh/depth rigging, explicit eye/mouth part segmentation, and physical-device tuning against native-camera tracking.
