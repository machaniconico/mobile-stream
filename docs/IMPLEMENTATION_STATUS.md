...
- Physical validation evidence now records explicit physical-device identity proof, rejects Simulator/Emulator/browser/generic test-device labels, surfaces physical iOS/Android coverage in Web/React Native diagnostics, and requires support bundle v23 physical-device plus current destination/protocol scope, stable monitor-hold sample/duration/drop/reconnect, native frame/byte/compositor asset, mic/headphone native monitor write/drop/latency, fresh avatar-tracking runtime, still-image rig-quality, spoken chat-readout, and YouTube/Twitch dashboard identity/state manifest proof before commercial release approval.
