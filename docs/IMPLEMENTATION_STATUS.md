...
- Physical validation evidence now records explicit physical-device identity proof, rejects Simulator/Emulator/browser/generic test-device labels, surfaces physical iOS/Android coverage in Web/React Native diagnostics, and requires support bundle v16 physical-device plus fresh avatar-tracking runtime manifest proof before commercial release approval.
...
- Still-image VTuber face tracking runtime with native-camera frames on mobile, simulated frames on web, neutral calibration, dead-zone filtering, jump limiting, lost-face return tuning, geometry-based illustration rig inference, iOS/Android native compositor 2.5D motion scaling, tunable illustration rig regions, lightweight pseudo mesh deformation, hair/body follow-through, and eye/mouth deformation.
...
- IRIAM-style high-fidelity single-illustration auto-rigging beyond the current geometry-based lightweight still-image 2.5D deformation, including image-segmentation-based face/body part inference, true mesh/depth rigging, explicit eye/mouth part segmentation, and physical-device tuning against native-camera tracking.
