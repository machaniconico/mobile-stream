...
- Physical validation evidence now records explicit physical-device identity proof, rejects Simulator/Emulator/browser/generic test-device labels, surfaces physical iOS/Android coverage in Web/React Native diagnostics, and requires support bundle v37 physical-device plus current destination/protocol scope, passing private launch rehearsal, stable monitor-hold sample/duration/drop/reconnect, controlled weak-network quality automation, native frame/byte/compositor applied/skipped overlay and asset proof, VRM renderer/model/pose proof when VRM sources are present, mic/headphone native monitor write/drop/latency plus Bluetooth review proof when applicable, fresh avatar-tracking runtime, ready native face-landmark confidence, retained manifest PNGTuber prepared-count/rig-quality or VRM visible-count/native-renderer proof, spoken chat-readout, YouTube/Twitch dashboard identity/state manifest proof including Twitch title/category/language metadata, and same-run platform ingest proof before commercial release approval.
