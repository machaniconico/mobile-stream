...
- Physical validation evidence now records explicit physical-device identity proof, rejects Simulator/Emulator/browser/generic test-device labels, surfaces physical iOS/Android coverage in Web/React Native diagnostics, and requires support bundle v19 physical-device plus native frame/byte/compositor asset, fresh avatar-tracking runtime, still-image rig-quality, and spoken chat-readout manifest proof before commercial release approval.
