...
- VRM/VRoid source records now persist VRM/GLB model URIs, expose them in Web/React Native inspectors, can pick/prepare local `.vrm`/`.glb` files into native-readable storage, warn on missing/remote/relative/unsupported or non-model URIs, and include shared plus native binary GLB validators that detect VRM 1.0/0.x extensions, humanoid bone mappings, expression/blendshape metadata, and native runtime load evidence.
