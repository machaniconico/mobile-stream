...
- Physical validation native-runtime proof must match the current scene's native composition requirements, so a screen-only runtime snapshot cannot approve a PNGTuber/image overlay scene and incomplete or non-production VRM renderer backend/model/pose telemetry cannot approve a VRM/VRoid scene for release evidence.
