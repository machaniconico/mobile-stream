...
- Release evidence package creation that copies a passed RC report, support bundle, UI/RN/web artifacts, distribution manifests, dashboard evidence, and store-submission evidence into a standalone hash-verified archive directory for handoff or store-submission audit, with a package-level scan for unredacted OAuth tokens, stream keys, client secrets, device codes, and Authorization headers in text evidence.
...
`npm run release:evidence-package -- /path/to/release-candidate-verification.json --output <dir>` creates a standalone release evidence package directory with the RC report, support bundle, and every hashed artifact recorded by the report. `npm run verify:evidence-package -- <dir>` rechecks the package manifest, copied file hashes, and text-evidence privacy scan; `--verify-sources` additionally confirms the original workspace files still match the archived evidence.
