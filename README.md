...
- Physical validation evidence now retains mic FX/headphone monitor snapshots, native self-monitor write/drop proof, device output-route/headphone safety, lip-sync/audio-meter sample summaries, YouTube/Twitch chat readout snapshots, spoken-chat success/failure counts, and native publisher/compositor telemetry, and downgrades retained pass attempts when native publisher/compositor proof, native self-monitor proof, audio monitoring, or chat readout validation is incomplete.
