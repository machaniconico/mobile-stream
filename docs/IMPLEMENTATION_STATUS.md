...
- Local release-candidate verification command that fails closed on uncommitted source changes, requires store-release orchestration report evidence when store-submission checklist evidence is present, revalidates distribution/dashboard/final store-submission handoff manifests against the current commit, requires browser UI evidence to come from a loopback preview target with git commit/dirty-state provenance, fails fast on the commercial support-bundle gate before expensive source gates, then chains repository automation checks, native release-config checks, unit tests, web/RN typechecks, web build, web bundle-size verification, browser UI verification, and React Native iOS/Android bundles, with JSON audit evidence for git revision, dirty state, support-bundle SHA-256, generated web/RN/UI artifact hashes, release-configuration input hashes, gate outcomes, timings, and failure reasons.
- Saved release-candidate report audit that revalidates archived RC reports against the current commit, clean git state, required report/UI-evidence git provenance, support-bundle SHA-256 plus the support bundle's commercial release gate at the report timestamp, loopback browser UI evidence schema/freshness/text checks, generated artifact hashes, release-configuration input hashes, and required gate outcomes before commercial approval.
