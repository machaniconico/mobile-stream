...
export const formatStreamDiagnosticReport = (
  ...
  const formatted = [
    ...
    `- Native audio encoder recovery: attempts ${diagnostics.nativeRuntime?.audioProcessing?.encoderRecoveryAttemptCount ?? 0} / recovered ${diagnostics.nativeRuntime?.audioProcessing?.encoderRecoverySuccessCount ?? 0} / failed ${diagnostics.nativeRuntime?.audioProcessing?.encoderRecoveryFailureCount ?? 0} / consecutive ${diagnostics.nativeRuntime?.audioProcessing?.encoderRecoveryConsecutiveFailureCount ?? 0} / pending ${diagnostics.nativeRuntime?.audioProcessing?.encoderRecoveryPending === true ? "yes" : "no"} / retry ${diagnostics.nativeRuntime?.audioProcessing?.encoderRecoveryRetryAfterMs ?? 0}ms / suppressed buffers ${diagnostics.nativeRuntime?.audioProcessing?.encoderRecoverySuppressedInputBufferCount ?? 0} / dropped input frames ${diagnostics.nativeRuntime?.audioProcessing?.encoderRecoveryDroppedInputFrameCount ?? 0} / discarded queued frames ${diagnostics.nativeRuntime?.audioProcessing?.encoderRecoveryDiscardedQueuedFrameCount ?? 0}`,
  ...
  return {
    ...
    audioProcessing: audioProcessing
      ? {
          ...
          encoderRecoveryLastReason: redactStreamKeyOccurrences(
