...
describe("stream diagnostics", () => {
  ...
  it("summarizes YouTube dashboard status for validation evidence", () => {
    ...
    expect(diagnostics.platformPublishing.youtube?.broadcastPrivacyStatus).toBe("private");
  ...
  it("fails YouTube dashboard diagnostics when broadcast privacy differs from the app setting", () => {
    const scene = createDefaultScene();
    const profile = {
      ...createDefaultStudioProfile(),
      destination: {
    ...
    const diagnostics = createStreamDiagnostics(scene, profile, readiness, {
      ...
      nativeRuntime: {
        ...
        composition: {
          ...
          vrmMissingPoseCount: 1,
          vrmModelUriCount: 1,
          vrmRuntimeStatuses: ["missing"],
          message: ""
        },
  ...
  it("warns when VRM poses arrive but the native VRM renderer has not rendered the source", () => {
    ...
    expect(formatStreamDiagnosticReport(createStreamDiagnosticReport(diagnostics))).toContain(
      "Composition VRM renderer: unavailable / none / rendered 0/1 / models 0/1 / versions - / bones 0 / expressions 0 / primitives 0 / skinned 0 / joints 0 / morphs 0 / materials 0 / textures 0 / pose bones 0/0 / pose expressions 0/0 / missing 1 / failed 0"
    );
  });
