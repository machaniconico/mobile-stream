...
- Store submission draft/checklist tooling for App Store / Play Console metadata JSON and iOS/Android store screenshots, including metadata generation from real-device screenshots or UI evidence, required listing/privacy/data-safety checks, byte/SHA-256 recording, secret-like text detection, verification, and optional release-candidate/report artifact capture when the checklist exists.
