...
const validationRunNativeRuntimeLabel = (run: StreamValidationRun): string | null =>
  run.nativeRuntime
    ? `native ${run.nativeRuntime.status} / ${run.nativeRuntime.platform} / publisher ${run.nativeRuntime.publisherState || "-"} / queue ${run.nativeRuntime.queuedItems}/${run.nativeRuntime.cacheSize} / ${liveRenderGraphMetricLabel(run.nativeRuntime)} / ${live2dPoseMetricLabel(run.nativeRuntime)} / frame interval ${run.nativeRuntime.videoFrameIntervalSampleCount} samples avg ${run.nativeRuntime.videoFrameIntervalAverageMs}ms max ${run.nativeRuntime.videoFrameIntervalMaxMs}ms jitter ${run.nativeRuntime.videoFrameIntervalJitterMs}ms / vrm ${run.nativeRuntime.vrmActivePoseCount}/${run.nativeRuntime.vrmSourceCount} active payloads ${run.nativeRuntime.vrmPosePayloadCount} renderer ${run.nativeRuntime.vrmRendererStatus} ${run.nativeRuntime.vrmRenderedSourceCount}/${run.nativeRuntime.vrmSourceCount} models ${run.nativeRuntime.vrmModelLoadedCount} bones ${run.nativeRuntime.vrmHumanoidBoneCount} expressions ${run.nativeRuntime.vrmExpressionCount} ${vrmRenderabilityMetricLabel(run.nativeRuntime)} pose ${run.nativeRuntime.vrmPoseBoneAppliedCount}/${run.nativeRuntime.vrmPoseBoneCount} bones ${run.nativeRuntime.vrmPoseExpressionAppliedCount}/${run.nativeRuntime.vrmPoseExpressionCount} expressions / continuity ${run.nativeRuntime.continuityStatus} incidents ${run.nativeRuntime.videoStallCount} video ${run.nativeRuntime.audioStallCount} audio max ${run.nativeRuntime.maxVideoStallDurationMs}ms/${run.nativeRuntime.maxAudioStallDurationMs}ms / A/V ${run.nativeRuntime.avSyncStatus} ${run.nativeRuntime.avSyncSkewMs}ms max ${run.nativeRuntime.avSyncMaxAbsSkewMs}ms incidents ${run.nativeRuntime.avSyncIncidentCount} critical ${run.nativeRuntime.avSyncCriticalIncidentCount}`
    : null;
...
const LiveCaptionPanel = ({
  ...
  return (
    <Panel title="Live Captions">
      <View style={styles.grid2}>
        <ActionButton
          label={liveCaption.settings.enabled ? "Captions On" : "Captions Off"}
          variant={liveCaption.settings.enabled ? "active" : "default"}
          onPress={() => onSettingsChange({ enabled: !liveCaption.settings.enabled })}
        />
        <View style={[styles.platformStatusBadge, liveCaption.status === "listening" && styles.platformStatusReady]}>
          <Text style={[styles.platformStatusText, liveCaption.status === "listening" && styles.platformStatusReadyText]}>
            {liveCaption.status}
          </Text>
        </View>
      </View>
      <Label text="Language" />
      <TextInput
        value={liveCaption.settings.language}
        onChangeText={(language) => onSettingsChange({ language })}
        style={styles.input}
        placeholder="ja-JP"
        placeholderTextColor="#71717a"
      />
      ...
      <View style={styles.grid3}>
        <ActionButton
          label={liveCaption.settings.interimResults ? "Interim On" : "Interim Off"}
          variant={liveCaption.settings.interimResults ? "active" : "default"}
          onPress={() => onSettingsChange({ interimResults: !liveCaption.settings.interimResults })}
        />
        <ActionButton label="Test Caption" disabled={!liveCaption.settings.enabled} onPress={onTestCue} />
        <ActionButton label="Clear" disabled={liveCaption.cues.length === 0} onPress={onClear} />
      </View>
      {liveCaption.errorMessage ? <Text style={styles.platformConnectionMessage}>{liveCaption.errorMessage}</Text> : null}
      {latestCue ? (
        <View style={styles.chatHistoryRow}>
          <Text style={styles.chatAuthor}>{latestCue.speaker || "Caption"}</Text>
          <Text style={styles.chatBody}>{latestCue.text}</Text>
        </View>
      ) : (
        <Text style={styles.mutedText}>No caption cues</Text>
      )}
    </Panel>
  );
};
