...
- Release evidence package creation that copies a passed clean RC report, support bundle, dedicated UI evidence JSON from either skipped or in-process browser UI gates, UI/RN/web artifacts, distribution manifests, dashboard evidence, and store-submission evidence into a standalone hash-verified archive directory for handoff or store-submission audit, rejects development-only dirty-worktree, commit-mismatch, or warning-approved RC reports, re-runs the archived support bundle's commercial release gate at the RC report timestamp with zero accepted warnings while keeping Go Live preflight warnings, public-launch checklist warnings, stale validation evidence, and incomplete chat overlay, text overlay, and live-caption evidence fail-closed, revalidates package generation time, package/report/UI full git commit and dirty-state provenance, packaged AAB/IPA ZIP content, packaged dashboard PNG/status JSON content, packaged real-device store screenshot PNG content/device/OS labels, packaged store metadata/review content, and packaged UI evidence schema/freshness/text/screenshot metadata, fails closed when commercial distribution/dashboard/store-submission artifact groups or manifest-referenced payloads are missing and when unmanifested package files are present, and runs package-level credential scans for unredacted OAuth tokens, stream keys, client secrets, device codes, and Authorization headers in text evidence and generated RN bundles.
