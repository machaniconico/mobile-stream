...
- Still-image VTuber face tracking runtime with native-camera frames on mobile, simulated frames on web, neutral calibration, dead-zone filtering, jump limiting, lost-face return tuning, image-aspect-aware geometry rig inference, browser alpha-mask foreground analysis for transparent/padded PNG auto-rigging, semantic face-landmark auto-rig input for detector/native ML handoff, production rig-line diagnostics, support-bundle rig quality score/grade evidence, iOS/Android native compositor 2.5D motion scaling, tunable illustration rig regions, lightweight pseudo mesh deformation, hair/body follow-through, and eye/mouth deformation.
...
- IRIAM-style high-fidelity single-illustration auto-rigging beyond the current image-aspect-aware geometry plus alpha-mask foreground and semantic face-landmark lightweight still-image 2.5D deformation, including semantic image-segmentation-based face/body part inference, true mesh/depth rigging, explicit eye/mouth part segmentation, and physical-device tuning against native-camera tracking.
