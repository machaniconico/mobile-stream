...
- Physical validation evidence now records explicit physical-device identity proof, rejects Simulator/Emulator/browser/generic test-device labels, surfaces physical iOS/Android coverage in Web/React Native diagnostics, and requires support bundle v21 physical-device plus stable monitor-hold sample/duration/drop/reconnect, native frame/byte/compositor asset, mic/headphone native monitor write/drop/latency, fresh avatar-tracking runtime, still-image rig-quality, and spoken chat-readout manifest proof before commercial release approval.
