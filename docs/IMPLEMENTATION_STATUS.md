...
- iOS host app runtime telemetry polling from the Broadcast Upload Extension, feeding bitrate, FPS, drops, reconnect attempts, errors, composition status, and still-image asset loaded/missing/decoded/pixel counts into the React Native snapshot.
...
- iOS ReplayKit Broadcast Upload Extension compositor for PNGTuber still-image/fallback, text, chat, image, and solid overlays on encoded frames, with App Group runtime composition status and still-image asset load/miss/decode pixel evidence.
