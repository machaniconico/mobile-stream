...
- Diagnostics reports and support bundles surface platform API audit counts, YouTube/Twitch dashboard status freshness, YouTube broadcast privacy mismatch checks, and YouTube bound-stream mismatch checks so release-candidate evidence can prove whether platform checks were current.
