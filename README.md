...
- Release evidence package creation that copies a passed RC report, support bundle, UI/RN/web artifacts, distribution manifests, and dashboard evidence into a standalone hash-verified archive directory for handoff or store-submission audit, with a package-level scan for unredacted OAuth tokens, stream keys, client secrets, device codes, and Authorization headers in text evidence.
